...
One can also write down a gauge-invariant dressed correlator between two vertices $v$ and $w$ connected by a path $\gamma_{vw}$:
\begin{equation}\label{eqn:dressed-time-correlator}
	...
	\right\rangle .
\end{equation}
Here $\tau_v$ and $\tau_w$ denote the local time-orientation labels at the endpoints, while the Wilson line makes the comparison gauge-invariant. However, the specific path-product realization explored in our present numerics is not retained as a reliable deconfinement diagnostic: after implementing and testing it, we found that the resulting dressed correlator is strongly suppressed in thermalized states and fails to cleanly distinguish phases, consistent with the constraints of Elitzur's theorem on local orientation variables. Accordingly, the quantitative transition diagnostics in this work should be understood as resting on the standard non-local gauge observables, especially Wilson loops and, for the finite-temperature lattice analysis, the Polyakov loop.
