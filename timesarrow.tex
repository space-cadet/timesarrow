...
\todo{Discuss work by Yoshida on the stability of SPT states at non-zero temperature and possible relation to emergence of stable geometry. \cite{Roberts2017Symmetry-protected,Yoshida2011Feasibility}}
...
\section*{Abstract}
{\bf
The question of the origin of time's arrow is a major outstanding problem in physics. Here we present a mechanism for the emergence of a cosmological arrow of time from a phase transition in the quantum gravitational vacuum. Following Chen and Vishwanath \cite{Chen2015Gauging}, who showed that time-reversal symmetry can be gauged on tensor network states, and using the correspondence between tensor networks and the spin-network states of Loop Quantum Gravity \cite{Qi2013Exact,Han2016Loop}, we introduce a $ Z_2 $ lattice gauge field on spin networks encoding a local time-reversal symmetry. We show that the effective theory of this gauge field is a $ Z_2 $ lattice gauge theory whose phase structure contains a confined phase -- corresponding to a pre-geometric ``quantum gravitational foam'' with no coherent arrow of time -- and a deconfined phase -- corresponding to semiclassical spacetime with a uniform cosmological arrow. The emergence of the arrow of time is identified with the confinement-deconfinement transition, detected by the Wilson loop order parameter, in a manner fully consistent with Elitzur's theorem. The deconfined phase is shown to correspond to a symmetry-protected topological (SPT) phase of the CZX type, whose topological order protects the coherent time orientation against local perturbations. We conjecture that the gapless edge modes of this SPT phase give rise to fermionic matter degrees of freedom.
}
...
We can extend this reasoning to two and three dimensional systems. In this case the site tensors will have three or more indices and a matrix insertion will be required on every leg \todo{Insert figure showing matrix insertion of tensor legs}. When applied to a subregion of a given lattice, the result will be that matrices on the interior will multiply to give the identity and we will be left with the non-trivial unitaries only on the boundary of the region as shown in \autoref{fig:symmetry-flux-line}.
...
There are two commonly used versions of the volume operator in LQG. They are named after the respective authors as the Rovelli-Smolin (RS) \cite{Rovelli1994Discreteness} or the Ashtekar-Lewandowski (AL) \cite{Ashtekar1996Quantum,Ashtekar1997cQuantum} versions. These have the form:
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Mapping classical volume to a quantum expression involves replacing classical vectors with angular momentum operators. Figure credit \cite{Vaid2014LQG-for-the-Bewildered}.}
...
The physical states live in the two copies of scalar (``s-wave'') states. Two non-orthogonal states which span this Hilbert space can be written as \cite{Zanardi1997Noiseless}:
\begin{align}
	\ket{\Psi_1} &  = \onehalf\left( \ket{1001} - \ket{0101} + \ket{0110} - \ket{1010} \right) \\
	\ket{\Psi_2} &  = \onehalf\left( \ket{1001} - \ket{0011} + \ket{0110} - \ket{1100} \right)
\end{align}
We can construct two orthogonal basis states for the spin-singlet Hilbert space by orthogonalizing the above two states.

The two states which span this two dimensional invariant subspace can be written as:
\begin{align}\label{eqn:vol-states}
	\ket{\Psi_1} & = \ket{\Psi_A} + \ket{\Psi_B} \nonumber \\
	\ket{\Psi_2} & = \ket{\Psi_A} - \ket{\Psi_B}
\end{align}
where:
\begin{align}\label{eqn:vol-states-2}
	\ket{\Psi_A} = \ket{0101} + \ket{1010} \nonumber \\
	\ket{\Psi_B} = \ket{1001} + \ket{0110}
\end{align}
Here the $ \ket{0}, \ket{1} $ are the two possible states of each edge carrying a spin $ 1/2 $ label. One can then write down the effective degrees of freedom carried by a four-valent intertwiner in terms of a a single spin $ 1/2 $ state $ \ket{\tilde\Psi} $ spanned by the invariant basis states $ \{\ket{\Psi_1}, \ket{\Psi_2}\} $:
\begin{equation}\label{key}
	\ket{\tilde \Psi} = \alpha \ket{\Psi_1} + \beta \ket{\Psi_2}
\end{equation}.
The states $ \{\ket{\Psi_1}, \ket{\Psi_2}\} $ can also be expressed in a manner which makes the connection with tensor network states apparent. Any state of $ N $ spins can be written as:
...
\begin{align}\label{eqn:vol-states-3}
	\ket{\Psi_1} & = i_1^{ABCD}\ket{i_A i_B i_C i_D} \nonumber \\
	\ket{\Psi_2} & = i_2^{ABCD}\ket{i_A i_B i_C i_D}
\end{align}
...
\begin{figure}[thbp]\label{fig:czx-entangled}
	\centering
	\includegraphics[width=0.2\linewidth]{czx_plaquette}
	\captionof{figure}{Each plaquette is in an entangled state: $ \ket{0000} + \ket{1111}$}
\end{figure}
...
\begin{figure}[h]
	...
	\begin{subfigure}[t]{0.3\textwidth}
		...
		\caption{Four 4-valent vertices are shown. In each vertex, one spin is shown in a different color (orange). The four orange spins form a multipartite entanlged state as shown in the next figure.}
...
Now if we change the sign of the triad field, this results in changing the sign of the determinant $ \det({}^3 e) $ of the triad field as well\footnote{For an $ N \times N $ matrix $ \mb{A} $, changing the matrix elements $ \mb{A} \rightarrow -\mb{A} $ the change in the sign of the determinant is given by $ \det(\mb{A}) = (-1)^N \det(-\mb{A}) $}. This implies that the volume of the classical geometry described by the given spin network also changes sign:

\begin{equation}\label{eqn:volume-sign}
	\text{Vol} = \int d^3 x \, \det({}^3 e) \rightarrow -\text{Vol}
\end{equation}
...
The identification of the deconfined phase with a topological quantum error correcting code \cite{Dennis2002Topological} provides an appealing physical picture for the stability of the cosmological arrow of time. In the deconfined phase, the $ Z_2 $ gauge theory on the spin network is equivalent to Kitaev's toric code, which is known to be a topological quantum memory: quantum information stored in its ground state is protected against local errors (bit flips and phase flips) up to a critical error rate.
...
\begin{itemize}
	...
	\item A rigorous analysis of whether the gapless edge modes of the SPT phase satisfy the Dirac equation in the semiclassical limit, which would establish the conjecture that fermionic matter emerges from the topological structure of quantum geometry.
	\item An investigation of the spin-foam history generated by the confinement-deconfinement transition, which would provide a four-dimensional description of the cosmological phase transition that gives rise to the arrow of time.
\end{itemize}
...
\[ g_{\mu\nu} = e_\mu^We e_\nu^J \eta_{IJ} \]
where $ \eta_{IJ} = \text{diag}(-1,1,1,1) $ is the flat Minkowski metric. In the so-called time gauge the time-time component of the above metric \eqref{eqn:adm-metric} can be used to determine the tetrad components for $ e_0^We $:
...
\end{equation}
The tetrad components for $ e_i^We $, $ i \in \{1,2,3\} $ then can be chosen to be:
...
\gamma_We e_\mu^We \left[ \partial^\mu + \frac{i}{2} \omega^\mu_{KL} \tau^{KL} \right] \psi + m \id \psi = 0,
\end{equation}
...
\end{equation}
where $ \kappa = 8 \pi G_N $, $ R $ is the Ricci scalar and $ g = \det(g) $ is the determinant of the 4-metric. Expressed in terms of the tetrad and connection variables \cite[Sec 4.5.3]{Vaid2014LQG-for-the-Bewildered} the action becomes:
\begin{align}
%\label{eqn:eh-tetrad-action}
S_{P}[e, \omega] & =\frac{1}{2 \kappa} \int d^{4} x \star\left(e^{I} \wedge e^{J}\right) \wedge F^{K L} \epsilon_{We we K L} \nonumber \\
& = \frac{1}{4 \kappa} \int d^{4} x \epsilon^{\mu \nu \alpha \beta} \epsilon_{We J K L} e_{\mu}^{I} e_{\nu}^{J} F_{\alpha \beta}^{K L} \nonumber \\
& = \frac{1}{4 \kappa} \int d^{4} x (\det {}^4 e) R[e]
\end{align}
where $ F_{\alpha \beta}^{K L} $ is the curvature of the spin-connection $ \omega_\mu^{IJ} $ and $ \det({}^4 e) $ is the determinant of the \emph{tetrad} field. From the form of this action and the relation \eqref{eqn:lapse-tetrad}, which implies that $ \det e \propto \text{sign}(N(t))$, we can see that the sign of the tetrad action $ S_P $ depends on the sign of the lapse. The sign of the lapse, however, has no effect on the sign of the determinant of the 4-metric and therefore does not affect the action given in \eqref{eqn:eh-metric-action}. One can see this more concretely by noting that in terms of the ADM lapse and shift fields, the 4-metric can be expressed as follows \cite[Eq (4.14)]{Vaid2014LQG-for-the-Bewildered}:
...
\begin{enumerate}[label=\Alph*.]
	\item \textbf{From metric to connection variables:}
...
%		\label{eqn:eh-metric-action}
		...
		\end{equation}
		where $ \kappa = 8 \pi G_N $, $ R $ is the Ricci scalar and $ g = \det(g) $ is the determinant of the 4-metric, we perform a canonical transformation \cite{Ashtekar1986New,Ashtekar1987New} from the four-metric and Christoffel connection being our dynamical variables $ \{g_{\mu\nu}, \Gamma_{\alpha\beta}^\delta \} $ to a new set of variables $ \{ e_\mu^I, A_\mu^{IJ} \} $ (known as the Ashtekar or Ashetkar-Barbero variables), where $ e_\mu^We $ is a tetrad field (choice of local Lorentz frame) and $ A_\mu^{IJ} $ is a $ \mf{so}(3,1) $ Lorentz gauge connection.
		\item In the new variables the Einstein-Hilbert action can be written in the form:
		\begin{align}\label{eqn:eh-tetrad-action}
		S_{P}[e, A] & =\frac{1}{2 \kappa} \int d^{4} x \star\left(e^{I} \wedge e^{J}\right) \wedge F^{K L} \epsilon_{We J K L} \nonumber \\
		& = \frac{1}{4 \kappa} \int d^{4} x \epsilon^{\mu \nu \alpha \beta} \epsilon_{We J K L} e_{\mu}^{I} e_{\nu}^{J} F_{\alpha \beta}^{K L}
		\end{align}
		...
		\item One can now perform a Legende transformation to obtain the Hamiltonian for general relativity in connection variables:
	...
	\end{enumerate}
	\item \textbf{From connection gravity to graph states}
	\begin{enumerate}[label*=\arabic*]
		...
		h(A) = \int_\gamma dx^\mu A_\mu^I(x) \tau_We 
		\end{equation}
		...
		\begin{namedthm*}{Peter-Weyl Theorem}
			The matrix elements of the Wigner representation matrices of a compact Lie group are othogonal \wrt the Haar measure on the group:
		...
		\end{equation}
	\end{enumerate}
	\item \textbf{From graph states to spin networks}
	\begin{enumerate}[label*=\arabic*]
		...
		\end{equation}
		Succintly, this is nothing more than the statement that \emph{the total angular momentum carried by edges into any vertex must vanish in the absence of matter}. Somewhat less transparent is the statement that physical states must live in the $ SU(2) $ invariant subspace of each vertex $ \mc{H}_v $:
		...
		\sum_{e \in E_v} \mb{J}^e_i \ket{\Psi_\Gamma} = 0, \quad \forall~v \in \Gamma
		\end{equation}
		%		More precisely we can write down the area of an arbitrary two-dimensional spatial surface $ S $ in terms of the electric field:
		%		\begin{equation}\label{eqn:classical-area}
		%		A_S = \int_S d^x~ \sqrt{E^a_i E^b_j n_a n_b \delta^{ij}}
		%		\end{equation}
	\end{enumerate}
    \item \textbf{Intertwiner Degrees of Freedom}
   	\begin{enumerate}[label*=\arabic*]
...
Given $ n $ particles transformation under difference representations of $ SU(2) $, \ie labeled by different spins $ \{j_1,\ldots,j_n \} $, an ``invariant tensor'' is a state in the tensor product state space: $ \mc{H}^n = \underset{i=1..n}{\bigotimes} \mc{H}^i $; where $ \mc{H}^i $ is the $ 2(j_i+1) $ dimensional space for a spin $ j_i $ particle. States in $ \mc{H}^n $ can be written in terms of the canonical basis:
