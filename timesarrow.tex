...
\begin{equation}\label{eqn:wilson-loop-expectation}
	\expect{\mc{O}_\gamma} = \expectop{\Psi}{\exp -i\int_{\gamma} dx^\mu \mb{A}_\mu}{\Psi}
\end{equation}
Such phases are characterized by invariants of fun
...
As a simple illustration of this consider the case of single vertex with four edges \autoref{fig:spin-net-vertex} . For simplicity, we set $ j = 1/2 $ for all edges. Now the state space $ \mc{H}^2 $ of a single edge is spanned by the states $ \{\ket{\uparrow}, \ket{\downarrow} \} $, or equivalently $ \{ \ket{1}, \ket{0} \} $. The state space of all edges taken together is then: $ \mc{H}^v = \{\mc{H}^2\}^{\otimes 4} $ and is spanned by states of the form $ \ket{j_1, j_2, j_3, j_4} $ where the coefficients $ j_k $ take values in $\{0,1 \}$. An arbitrary state of the edge system can then be written as:
\begin{equation}\label{eqn:tetra-edge-wf}
\ket{\Psi_e} = \sum_{\{j_k\}} C_{j_1, j_2, j_3, j_4} \left(\mc{U} \ket{j_1, j_2, j_3, j_4}\right)
\end{equation}
where the sum is over all spin states of the edges and $ C_{j_1, j_2, j_3, j_4} $ is the weight for the corresponding state vector. Now consider a gauge transformation acting locally on each edge. The state of the spin on the $ k^\text{th} $ edge undergoes a $ SU(2) $ rotation, given by:
\begin{equation}\label{eqn:su2-rotation}
	\ket{j_k} \rightarrow \ket{j'_k} = e^{-i\hat{\alpha_k}\cdot\vec{\sigma}} \ket{j_k}
\end{equation}
...
\begin{equation}\label{eqn:rotated-state}
	\ket{\Psi'} = \sum_{\{j_k\}} C_{j_1, j_2, j_3, j_4} \left(\mc{U} \ket{j_1, j_2, j_3, j_4}\right)
\end{equation}
...
\begin{equation}\label{eqn:group-average}
	\ket{j_1, j_2, j_3, j_4}{}_{Inv} = \int \left\{\prod_{k = 1..4} d\alpha_k\right\} ~ \mc{U}(g) ~ \ket{j_1, j_2, j_3, j_4}
\end{equation}
...
\subsection{Spin Networks as Gauged Tensor Networks}\label{subsec:spin-nets-tensors}
