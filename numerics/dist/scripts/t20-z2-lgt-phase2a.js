 ...
 */
import { createSquareLattice, Z2GaugeField, averagePlaquetteMoments, metropolisSweep, thermalize, jackknifeError, binData, susceptibility, plaquetteSpecificHeat, binderCumulant } from 'ts-quantum';
...
function runSimulation(params) {
    ...
    const uBinned = pBinned.map((_, i) => binderCumulant(Array(pBinned.length).fill(0).map((_, j) => j === i ? pBinned[j] : pBinned[j]) // hacky - need raw values
    ));
