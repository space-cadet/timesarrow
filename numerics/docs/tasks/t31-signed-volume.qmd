...
::: {.callout-warning}
## Historical Page
This page is now primarily a provenance record. The signed-volume path was not retained as the active deconfinement diagnostic. The current T31 path uses the Polyakov loop instead: **[T31 — Deconfinement via Polyakov Loop](t31-deconfinement-polyakov.html)**.
:::
...
| Phase 3 | Gauge-invariant dressed correlator | ❌ Failed as practical order parameter | 2026-07-14 |
| Phase 4 | Pivot to Polyakov loop | ✅ Complete | 2026-07-14 |
...
Historically this suggested that the confinement-deconfinement transition might be tested through:
...
The replacement implemented in `rust-lattice` was a dressed orientation correlator:
...
is designed to be invariant under local Z₂ gauge transformations. In source-level tests it is gauge-invariant, but in thermalized runs it failed to cleanly separate phases and was therefore not retained as the active T31 observable.
...
These source-level tests now pass under the Rust 2024-compatible validation path. The reason this page is superseded is not a missing code fix, but the physical negative result: the dressed correlator remains suppressed in thermalized states and does not provide a reliable deconfinement signal.

## Next Steps

1. Keep the signed-volume data and dressed-correlator implementation as provenance and negative-result context.
2. Use the Polyakov loop page for the active T31 deconfinement program.
3. Only revisit signed-volume-style observables if a materially different gauge-invariant construction is proposed.
