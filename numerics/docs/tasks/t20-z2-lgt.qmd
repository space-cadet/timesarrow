...
date: "2026-06-25"
date-modified: "2026-06-26T03:12:00+05:30"
---

*Last updated: 2026-06-26 03:12 IST*
...
![3D Plaquette vs β (multi-lattice)](../assets/t20-p3-plaquette-vs-beta-multi.png)

*Figure 3: Plaquette expectation value vs coupling β for L = 4, 6, 8 (3D cubic, 21 β values). The dashed vertical line marks the critical point β_c ≈ 0.76. Curves steepen with increasing L, approaching a step function in the thermodynamic limit.*

![3D Specific Heat vs β (multi-lattice)](../assets/t20-p3-specific-heat-vs-beta-multi.png)

*Figure 4: Specific heat C_V vs β (multi-lattice). The peak grows with L and shifts toward β_c, characteristic of a first-order transition in 3D.*

![3D Susceptibility vs β (multi-lattice)](../assets/t20-p3-susceptibility-vs-beta-multi.png)

*Figure 5: Susceptibility χ vs β (multi-lattice). The divergence sharpens with L, with peak shifting from β ≈ 0.68 (L=4) to β ≈ 0.74 (L=8).*

![3D Binder Cumulant vs β (multi-lattice)](../assets/t20-p3-binder-vs-beta-multi.png)

*Figure 6: Binder cumulant U vs β (multi-lattice). The horizontal dotted line marks the 3D Ising universal value U* = 2/3 ≈ 0.667. All three curves approach this value from below in the ordered phase, with the crossing sharpening toward β_c as L increases.*

![3D Combined Overview (multi-lattice)](../assets/t20-p3-combined-multi.png)

*Figure 7: Combined overview of all four observables for 3D Z₂ LGT (fine-grained, 21 β values, multiple L). The critical region β ≈ 0.70–0.80 shows strong signals across all observables, with finite-size effects clearly visible.*
...
### Binder Cumulant First-Order Analysis

**Date**: 2026-06-26

The 3D Z₂ LGT transition is **first-order** (distinct from the continuous 2D transition). This is revealed by the Binder cumulant behavior: rather than curves crossing at a critical point (second-order signature), the cumulant shows a **minimum** that deepens with increasing lattice size, with the high-β plateau approaching $U = 2/3$.
...
**Fit result**: $U_{\min}(L) = 2/3 - 2.36 \cdot L^{-3.084}$, with exponent $-3.084$ within 3% of the expected $-3$ for a first-order transition in 3D. The extrapolation to $L \to \infty$ gives $U_{\infty} = 0.6697$, within 0.5% of $2/3$.

![Binder cumulant first-order signature](../assets/t20-binder-first-order.png)

*Figure 10: Binder cumulant $U_L(\beta)$ for L = 4, 6, 8 (3D cubic). The minimum deepens with increasing L, while the high-β plateau approaches $U = 2/3$ (dotted line). The star markers indicate the minimum positions. This is the hallmark of a first-order transition.*

![Binder cumulant scaling analysis](../assets/t20-binder-first-order-scaling.png)

*Figure 11: Left: minimum depth $U_{\min}(L)$ versus $L$, showing convergence to $2/3$ with finite-size corrections. Right: log-log plot confirming $U_{\min}(L) \sim L^{-d}$ scaling with fitted exponent $-3.084 \approx -3$.*

### Critical Exponent & Scaling Collapse (Preliminary)

**Note**: With only $L = 4, 6, 8$, finite-size scaling is not yet in the asymptotic regime. Proper exponent extraction requires $L \geq 16$–$32$. The preliminary results below are consistent with this limitation.

| Method | Fitted | 3D Ising | Status |
|--------|--------|----------|--------|
| γ/ν from χ_max | 0.370 | 1.963 | ⚠️ Underestimated (small L) |
| α/ν from C_max | 0.493 | 0.175 | ⚠️ Overestimated (small L) |
| ν from β_c shift | 0.522 | 0.630 | ⚠️ Underestimated (small L) |
| Scaling collapse | Poor | — | ⚠️ Need L ≥ 16 |

**Conclusion**: The small lattice sizes ($L = 4, 6, 8$) are insufficient for reliable critical exponent extraction. The Binder cumulant first-order scaling (above) is robust with current data, but universality-class confirmation via scaling collapse requires larger lattices ($L = 16, 24, 32$). These runs are fast in Rust (~seconds per L) and will be completed in the next validation session.

![Scaling collapse test](../assets/t20-scaling-collapse.png)

*Figure 12: Scaling collapse test using literature 3D Ising exponents (ν = 0.630, γ = 1.237). Left: susceptibility rescaled as $L^{-\gamma/\nu} \chi$ versus $L^{1/\nu}(\beta - \beta_c)$. Right: order parameter rescaled as $L^{\beta/\nu} \langle P \rangle$. The curves do not collapse with current $L = 4, 6, 8$ data, confirming the need for larger lattices.*
