...
async function runParallelSweeps(config) {
  ...
  if (remainingBetas.length === 0) {
    if (verbose) console.log(`All ${betaValues.length} β values already completed!`);
    return results;
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    function assignWork(worker) {
      ...
      if (verbose) {
        ...
        console.log(`[${elapsed}] Starting β = ${beta} (${totalDone + 1}/${betaValues.length})`);
      }
    ...
    function onResult(result) {
      ...
      if (verbose) {
        const elapsed = formatTime(Date.now() - startTime);
        console.log(`[${elapsed}] β = ${result.beta}: ⟨P⟩ = ${result.meanPlaquette.toFixed(4)} ± ${result.errorPlaquette.toFixed(4)} (${formatTime(result.wallTimeMs)})`);
      }
    ...
    function checkDone() {
      if (activeWorkers === 0 && queue.length === 0) {
        ...
        if (verbose) console.log(`  All simulations complete in ${totalTime}`);
        if (verbose) console.log(`═══════════════════════════════════════════`);
        resolve(results);
      }
    }
...
if (require.main === module) {
  ...
  runParallelSweeps({ betaValues, params })
    .then(results => {
      ...
      console.log('═══════════════════════════════════════════');
      console.log('');
      console.log('| β | ⟨P⟩ | Error | Time |');
      console.log('|---|-----|-------|------|');
      results.forEach(r => {
        console.log(`| ${r.beta.toFixed(2)} | ${r.meanPlaquette.toFixed(4)} | ±${r.errorPlaquette.toFixed(4)} | ${formatTime(r.wallTimeMs)} |`);
      });
...
module.exports = { runParallelSweeps };
