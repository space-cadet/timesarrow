...
import {
  createSquareLattice,
  Z2GaugeField,
  averagePlaquetteMoments,
  metropolisSweep,
  thermalize,
  jackknifeError,
  binData,
  susceptibility,
  plaquetteSpecificHeat,
  binderCumulant
} from 'ts-quantum';
...
interface SimulationResult {
  ...
  meanPlaquetteSq: number;
  errorPlaquetteSq: number;
  meanPlaquetteFourth: number;
  errorPlaquetteFourth: number;
  susceptibility: number;
  errorSusceptibility: number;
  specificHeat: number;
  errorSpecificHeat: number;
  binderCumulant: number;
  errorBinderCumulant: number;
...
function runSimulation(params: SimulationParams): SimulationResult {
  ...
  const pBinned = binData(pValues, binSize);
  const p2Binned = binData(p2Values, binSize);
  const p4Binned = binData(p4Values, binSize);
  ...
  const chiBinned = pBinned.map((p, i) => susceptibility(p, p2Binned[i], volume));
  const cBinned = pBinned.map((p, i) => plaquetteSpecificHeat(p, p2Binned[i], beta));
  const uBinned = pBinned.map((_, i) => binderCumulant(
    Array(pBinned.length).fill(0).map((_, j) => j === i ? pBinned[j] : pBinned[j]) // hacky - need raw values
  ));
...
async function main() {
  ...
  const allParams: SimulationParams[] = [];
  ...
  for (let i = 0; i < allParams.length; i += MAX_PARALLEL) {
    ...
    const batchResults = await Promise.all(
      batch.map(params => runSimulation(params))
    );
