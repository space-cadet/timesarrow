...
async function runParallelSweeps(config: RunConfig): Promise<BetaResult[]> {
  ...
  const results: BetaResult[] = checkpoint?.results ? [...checkpoint.results] : [];
  const completed: number[] = checkpoint?.completed ? [...checkpoint.completed] : [];
  ...
  return new Promise((resolve, reject) => {
    ...
    const queue: number[] = [...remainingBetas];
    let activeWorkers = 0;
    ...
    function assignWork(worker: Worker) {
      ...
      if (verbose) console.log(`[${new Date().toISOString()}] Starting β = ${beta} (${totalDone + 1}/${betaValues.length})`);
      worker.postMessage({ ...params, beta });
      return true;
    }
    
    function onResult(result: BetaResult) {
      results.push(result);
    ...
    function onError(beta: number, error: string) {
