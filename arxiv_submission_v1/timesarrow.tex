...
% \usepackage[draft]{todonotes}  % Disabled for arXiv submission
...
% todonotes disabled for arXiv submission

\graphicspath{{figures/}}

\begin{document}

% Note: Yoshida on SPT stability at non-zero T and relation to stable geometry \cite{Roberts2017Symmetry-protected,Yoshida2011Feasibility}
...
% Author list
\begin{center}
Deepak Vaid\textsuperscript{*}\,\thanks{Source code and full revision history available at \url{https://github.com/space-cadet/timesarrow}}
...
% Line numbers disabled for submission
...
\end{equation}
One can explicitly evaluate the expression \eqref{eqn:mps-ansatz} with the choice of above matrices for, say, a two-site system to find that it does indeed correspond to \eqref{eqn:ghz-state}. It is important at this point to mention that it has been proven in \cite{Vidal2004Efficient} that \emph{any} many-body state can be written in the form \eqref{eqn:mps-ansatz} as long as the bond dimension $ d $ is large enough. Therefore the MPS ansatz can be used to study local application of time-reversal, among other, symmetries to an arbitrary many-body system.
...
\begin{enumerate}
	...
	\item \textbf{Thermal suppression}: At temperatures below the Planck scale ($ T < T_{\rm Planck} $, i.e.\ $ \beta > 2/\kappa $ where $ \kappa = 8\pi\gamma l_p^2 $), the thermal Boltzmann weight strongly favours $ j = 1/2 $ edges, since they carry the smallest non-zero area eigenvalue and hence the lowest energy cost. A direct computation of the single-edge partition function shows that the $ j = 1/2 $ contribution exceeds the combined contribution of all higher-$ j $ representations for $ \beta > \beta_c \approx 2/\kappa $ (see \autoref{subsec:czx-intertwiner} and the supplementary calculations). The crossover temperature $ T_c \approx 4\pi\gamma l_p^2 / k_B $ is of order the Planck temperature for $ \gamma \sim \mathcal{O}(1) $, so sub-Planck spin networks are dominated by $ j = 1/2 $ edges.
...
\subsection*{From graph states to spin networks}
	\begin{enumerate}[label=\arabic*.]
		...
		\end{equation}
		In the 3-valent or 4-valent cases, gauge invariance ensures the total flux cancels exactly, as shown in \autoref{sec:volume-z2}. For example, at a 4-valent vertex, $\sum_{i=1}^4 \vec{J}_i = 0$. Since each $\tau_v$ generator at vertex $v$ squares to unity, $\tau_v^2 = 1$, the state remains invariant under local transformations.
	\end{enumerate}
...
\begin{align}
  U_{CZ}\ket{\Phi_1} &= \frac{1}{2}\bigl(\ket{0101}+\ket{0110}+\ket{1001}+\ket{1010}\bigr).
\end{align}
One verifies that $\hat{S}^+_{\rm tot}(U_{CZ}\ket{\Phi_1}) \neq 0$, so this state lies \emph{outside} the $j_{\rm tot}=0$ subspace. The same holds for $U_{CZ}\ket{\Phi_2}$. Since $U_{CZX} = U_X\cdot U_{CZ}$ and $U_X$ preserves the singlet subspace while $U_{CZ}$ does not, $U_{CZX}$ does not preserve the singlet subspace.
